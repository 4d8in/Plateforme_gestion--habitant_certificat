...
<head>
    ...
    <title>Portail habitant - Mot de passe oublié</title>
...
<body class="min-h-screen bg-slate-900 text-slate-100 flex items-center justify-center px-4">
    <div class="w-full max-w-md card p-6">
        ...
        <form method="POST" action="{{ route('portal.forgot_password.post') }}" class="space-y-4">
            @csrf
            <div>
            ...
            <x-primary-button class="w-full justify-center">Générer le lien</x-primary-button>
        </form>

        <div class="mt-4 text-sm text-slate-300 flex flex-col gap-1">
            <a href="{{ route('portal.login') }}" class="text-sky-300 hover:underline">Retour connexion</a>
            <a href="{{ route('portal.set_password') }}" class="text-sky-300 hover:underline">Créer mon mot de passe</a>
        </div>
    </div>
</body>
</html>
